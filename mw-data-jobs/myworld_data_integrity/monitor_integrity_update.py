...
def fix(doc):
  ...
  failure_messages = [('Failed: ' + method.__doc__) for method in failedTests ]
  
  dict_unsetters = {}
  dict_setters = {}
  dict_originals = {}
  #fix
  disposable_fields = ['age', 'country', 'education', 'gender', 'sourceMethod']
  re_test = re.compile(r'Failed: test (\w+) field is valid')
  
  for message in failure_messages:
    if message == 'Failed: test validity of priority ids':
      dict_originals.update({'choices': doc['choices']})
      clean_set = set(doc['choices']).intersection(set(range(0,17)))
      doc['choices'] = [ choice for choice in clean_set ]
      dict_setters.update( {'choices': doc['choices']} )
    else:
      matched = re_test.search(message)
      if matched and matched.group(1) in disposable_fields:
        key = matched.group(1)
        dict_originals.update({key: doc[key]})
        dict_unsetters.update( {key: 1} )
  
  if dict_setters or dict_unsetters or dict_originals:
    dict_setters.update({'original_values': dict_originals})
    collection.update({'_id': doc['_id']}, \
                      {'$set': dict_setters, '$unset': dict_unsetters})
...
if __name__ == "__main__":
  ...
  print('Checking if new votes are available...')
  
  dict_condition = {'data_quality': {'$exists': False}}
  if len(sys.argv) > 1 and sys.argv[1] == 'all':
    print 'Scanning entire collection!'
    dict_condition = {}
  if not collection.find(dict_condition, snapshot = True).count():
    print('No new votes')
    sys.exit(0)
  
  print('Attempting to fix')
  #table scan to check ballots and to build dict_df
  cursor = collection.find(dict_condition, snapshot = True)
  docs = []
  progress_bar = 0
  progress_extent = cursor.count()
  for doc in cursor:
    if progress_bar % 200 == 0:
  ...
  print('Running tests on %d records...' % len(docs))
  # cursor = collection.find(dict_condition, snapshot = True)
  progress_bar = 0
  progress_extent = cursor.count()
