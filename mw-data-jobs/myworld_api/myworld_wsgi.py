...
DATA_QUALITY_THRESH = -3

mode = 'server'

run_modes = {'local' : {'host': 'localhost', 
                        'port': 3000,
                        'dir': '/Users/niko/Documents/Stahl/Repos/myworld-analytics/production_data/myworld_api/',
                        'config_dir': '/Users/niko/Documents/Stahl/Repos/myworld-analytics/production_data/myworld_api/data/'
                        },
...
collection = db['votes']
config_file = json.load(open(config_dir + 'config.json'))
...
region_indices = config_file['region_indices']
...
CACHE_ANALYTICS = 50*60
CACHE_VERY_SHORT = 5*60
CACHE_SHORT = 10*60
CACHE_LONG = 10*60
CACHE_INF = 0
...
#DOES NOT use build_query
#USES get_totals
#uses CACHE_ANALYTICS
...
def depr_totals(mc):
  ...
  if request.path == '/country':
    ...
    if "id" in query_keys:
      if int(request.query.id)>0 and int(request.query.id)<195:
    ...
    if "id" in query_keys:
      if int(request.query.id) in [1,2,3]:
        dict_filter['sourceMethod']['$in'] = [source_methods[str(request.query.id)]]
      else:
        return 'Not a valid id'
  
  to_return = get_totals( dict_filter , request)
  to_return['computed_at'] = int(ptime.time())
  mc.set(cache_key, to_return, CACHE_ANALYTICS)
  return to_return
...
#Add data quality
@app.route('/public/segment_comparison')
def segment_comparison(mc):
  
  filters = {
  'total': {},
  'gender1': {'gender': 1},
  'gender2': {'gender': 2},
  'ageGroup1': {'age': {'$gte': ageGroups['group1']['low'], '$lte': ageGroups['group1']['high']}},
  'ageGroup2': {'age': {'$gte': ageGroups['group2']['low'], '$lte': ageGroups['group2']['high']}},
  'ageGroup3': {'age': {'$gte': ageGroups['group3']['low'], '$lte': ageGroups['group3']['high']}},
  'hdi1': {'country': {'$in': hdi_to_country_map['1']}},
  'hdi2': {'country': {'$in': hdi_to_country_map['2']}},
  'hdi3': {'country': {'$in': hdi_to_country_map['3']}},
  'hdi4': {'country': {'$in': hdi_to_country_map['4']}}
  }
  
  for (filter_name, filter) in filters.items():
    filter.update({'data_quality': { '$gt': DATA_QUALITY_THRESH }})
  ...
  if not obj:
    ...
    for (name, filter) in filters.items():
      obj[name] = aggregate_ranking(filter)
...
def calc_segment(mc):
  ...
  dict_find = build_query(request)
  obj = aggregate_ranking(dict_find)
  obj['computed_at'] = int(ptime.time())

  #CACHE
  if request.path == '/public/segment':
    mc.set(cache_key, obj, CACHE_LONG)
  elif request.path == '/segment':
    mc.set(cache_key, obj, CACHE_ANALYTICS)
  
  #CSV
  if request.path == '/csv/segment':
    dict_norm = normalize_segment_view( dict_find, obj, request )
...
@app.route('/totalvotes', method='GET')
def get_total(mc):

  # check cache
  cache_key = make_cache_key(request)
  obj = mc.get(cache_key)
  if obj and "dev" not in request.query.keys():
    return obj

  # r = json.load(urllib2.urlopen("http://ec2-23-22-13-62.compute-1.amazonaws.com/stats2/segment"))
  r = json.load(urllib2.urlopen("http://54.227.246.164/stats2/segment"))
  ret = { "total": r["_total"], "computed_at": int(ptime.time()) }
  # cursor = collection.find({'data_quality': { '$gt': DATA_QUALITY_THRESH }})
  # ret = { 'total': cursor.count(), 'computed_at': int(ptime.time()) }

  mc.set(cache_key, ret, CACHE_SHORT)
  return ret
...
#DOES NOT use build_query
@app.route('/partners', method='GET')
def partners(mc):
  return get_partners()

#DOES NOT use build_query
@app.route('/partner_rankings', method='GET')
def get_partner_rankings(mc):
  cache_key = make_cache_key(request)
  obj = mc.get(cache_key)
  if obj:
    return obj
  ...
  #ensure data quality
  dict_condition = {'data_quality': { '$gt': DATA_QUALITY_THRESH }}
  #aggregate
  ...
  to_return = {'partner_rankings': results}
  mc.set(cache_key, to_return, CACHE_ANALYTICS)
  return to_return
    ...
    for doc in docs:
      try:
        ...
        if doc['overlap']> 3:
          ...
          doc['region_id'] = int(region)
          doc['choices'] = [ choice for choice in set(doc['choices']).difference(set([0])) ]
          results += [ doc ]
          break
      except TypeError:
        pass
...
#Add data quality
@app.route('/public/recent_votes', method='GET')
def recent_votes(mc):
  cache_key = make_cache_key(request)
  obj = mc.get(cache_key)
  if obj:
  ...
  dict_find = {'timestamp': {'$gt': datetime.now() - timedelta(days=1)},\
               'data_quality': {'$gt': DATA_QUALITY_THRESH}, 'country': {'$exists': True}, \
               'gender': {'$exists': True}, 'age': {'$exists': True} }
  
  dict_fields = {'_id': 0, 'choices': 1, 'gender': 1, 'country': 1, \
                 'age': 1}
  cursor = collection.find(dict_find, dict_fields) \
            .sort('timestamp', pymongo.DESCENDING) \
            .limit(50)
  
  to_return = {'results': []}
  for doc in cursor:
    if str(doc['country']) in country_metadata:
      doc['region_id'] = region_indices[country_metadata[str(doc['country'])]['region']]
      doc['choices'] = [ choice for choice in set(doc['choices']).difference(set([0])) ]
      to_return['results'] += [ doc ]
  
  mc.set(cache_key, to_return, CACHE_VERY_SHORT)
  return to_return
...
def www_counts():
  ...
  toreturn['people'] = collection.find().count({'data_quality': {'$gt': DATA_QUALITY_THRESH}})
...
def build_query(request):
  ...
  for client_key in query_keys:
    ...
    if client_key == 'partner':
     ...
     #Gayan - added this line    
     dict_find.update({'partner' :  request.query['partner']})
  ...
  #ensure data quality
  dict_find.update({'data_quality': {'$gt': DATA_QUALITY_THRESH}})
  return dict_find
...
def get_country_stats(dict_filter = {}, calcHDI = True):
  ...
  dict_condition = {"country" : {"$exists" : True}}
  dict_condition.update({'data_quality': {'$gt': DATA_QUALITY_THRESH}})
  dict_condition.update(dict_filter)
...
def get_totals(dict_filter = {}, request = None):
  ...
  dict_filter.update({'data_quality': {'$gt': DATA_QUALITY_THRESH}})
  ...
  if request and request.path != '/country':
    pass
  elif request and (request.path == '/abbr' or 
...
def get_tseries(mc, dates, dict_filter = {}):
  ...
  var_names = ['gender', 'education', 'country', 'partner']
  ...
  for bin_date in dates:
    ...
    for obj in date_stat['country']:
      if obj['_id'] and obj['_id'] != 'NA' and obj['_id']<195 and obj['_id']>0:
        meta = country_metadata[ str(int(obj['_id'])) ]
    ...
    rec_result = collection.aggregate([
                      ...
                      {"$unwind": "$choices"},
                      {"$group": {"_id": "$choices", "count": {"$sum": 1}}}])
    valid_ids = range(1,17)
    date_stat['recs'] = [ r for r in rec_result['result'] if r['_id'] in valid_ids ] # filter out invalid ids
...
def make_cache_key(request):
  to_return = request.urlparts[2] + '/' + request.urlparts[3]
  return to_return
...
############# LAUNCH BOTTLE APP #############
#############################################
#no longer necessary when using mod_wsgi apache server
if mode == 'local':
