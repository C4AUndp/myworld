...
def segment_comparison(mc):
  ...
  for (filter_name, filter) in filters.items():
    filter.update({'data_quality': { '$gt': -2 }})
...
def get_partner_rankings(mc):
  ...
  #ensure data quality
  dict_condition = {'data_quality': { '$gt': -2 }}
  #aggregate
...
def recent_votes(mc):
  ...
  dict_find = {'timestamp': {'$gt': datetime.now() - timedelta(days=1)},\
               'data_quality': {'$gt': -2}, 'country': {'$exists': True}, \
...
def www_counts():
  ...
  toreturn['people'] = collection.find().count({'data_quality': {'$gt': -2}})
...
def aggregate_ranking(dict_find = {}):
  ...
  for result in result_obj['result']:
    if int(result['_id']) in range(0,17):
...
def build_query(request):
  ...
  #ensure data quality
  dict_find.update({'data_quality': {'$gt': -2}})
  return dict_find
...
def get_country_stats(dict_filter = {}, calcHDI = True):
  ...
  dict_condition = {"country" : {"$exists" : True}}
  dict_condition.update({'data_quality': {'$gt': -2}})
  dict_condition.update(dict_filter)
...
def get_totals(dict_filter = {}, request = None):
  ...
  dict_filter.update({'data_quality': {'$gt': -2}})
